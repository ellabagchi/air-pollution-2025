...
out_nc = out_dir / (
    ...
    f"step{str(GRID_STEP_DEG).replace('.','p')}.nc"
)


# -------------------------
# Load train data and fit model
# -------------------------
...
for t in range(len(times)):
    ...
    df_feat = base_grid_df.copy()
    df_feat["year"] = y
    df_feat["month"] = m

    yhat = model.predict(df_feat[["Latitude", "Longitude", "year", "month"]]).astype(np.float32)
...
# ---- Coordinate metadata (helps for interpretation) ----
...
ds.attrs = {
    ...
    "methodology": (
        "A RandomForestRegressor model was trained using predictors: latitude, longitude, year, month. "
        "Monitoring locations were split into spatially disjoint training/validation/test sets. "
        "The trained model was applied to a regular grid to produce gridded monthly PM2.5 fields."
    ),
    "training_data": (
...
# -------------------------
# Write NetCDF
# Note: no compression here so it works with scipy backend
# -------------------------
# -------------------------
