#!/bin/bash
# this code works for downloading no2 daily data 1 year at a time-- change the year as needed, change variable (no2, 03, hcho) as needed
for year in 2014; do
    ...
    tmp_dir="./omi_o3_tmp_${year}"
    target_dir="/home/ellab/air_pollution/src/data/new_o3/${year}"
    ...
    for file in "$tmp_dir"/${prefix}*.he5; do
        if [[ -f "$file" ]]; then
            ...
            if [[ -n "$datetag" ]]; then
                ...
                m=${datetag:4:2}
                d=${datetag:6:2}
                new_filename="omi_o3_${y}${m}${d}.he5"
